...
pub trait BlockTraits<T> {
    fn validate(&self, publickey: &PublicKeyType, signature: &[u8]) -> bool;
...
impl BlockTraits<KeypairType> for Block {
    fn validate(&self, publickey: &PublicKeyType, signature: &[u8]) -> bool {
        ...
        PublicKey::verify(publickey, &ser_block, &signature)
...
impl SignedBlockTraits<KeypairType> for SignedBlock {
    fn validate(&self, publickey: &PublicKeyType) -> bool {
        ...
        PublicKey::verify(&publickey, &ser_block, &self.signature)
    }
...
mod tests_blocks {
    ...
    pub fn test_block() {
        ...
        let sign = block.sign(&kp);
        let validate = block.validate(&public_key, &sign);
        println!("{}", validate);
    }
}
