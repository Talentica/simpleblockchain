...
use schema::transaction::{CryptoTransaction, SignedTransaction};

//Sample Code to generate/serialize/deserialize the transactions using protobuf/schema::message.
pub fn create_transaction(nnce: u64, amt: u64) -> SignedTransaction {
    SignedTransaction{
...
pub fn serialize_transaction(txn: &SignedTransaction) -> Vec<u8> {
    let mut buf = Vec::new();
...
pub fn deserialize_transaction(buf: &[u8]) -> SignedTransaction {
    let txn = SignedTransaction::decode(&mut Cursor::new(buf)).unwrap();
    println!(
